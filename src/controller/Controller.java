...
//	private int aufgaben = -1, itAufgaben=-1, aktuelle = -1;
	...
	public void stop() {
		if(JOptionPane.showConfirmDialog(view, "Bist du sicher?", "Task Planner schließen", JOptionPane.OK_CANCEL_OPTION)==JOptionPane.OK_OPTION) {
	...
	private void actionLaden(ActionEvent e) {
		...
		case JFileChooser.APPROVE_OPTION:
			if(quellDatei.getSelectedFile()!=null) {
				...
				// Si no, no será compatible con DB
	...
	private void actionSpeichern(ActionEvent e) {
		if(!bearbeitenFenster.getNameJTF().getText().isEmpty()) {
			...
			if(!exist) {
				...
				JOptionPane.showMessageDialog(bearbeitenFenster, "Die Aufgabe ist bereits vorhanden, überprüfen Sie den Namen und das Projekt.", "Bearbeiten", JOptionPane.ERROR_MESSAGE);
			}
		}
		else {
...
//		datum2.setTime(montagDate);
	...
	private void actionZeitLoeschen(JButton ar) {
		if(JOptionPane.showConfirmDialog(bearbeitenFenster, "Bist du sicher?", "Zeit Löschen", JOptionPane.OK_CANCEL_OPTION)==JOptionPane.OK_OPTION) {
	...
	/*
	 * Man male ist die auswählte Reihe gemaltet und wir schrollen die Scrollbar. 
	 */
	/*
	...
	private void actionProjektSpeichern(ActionEvent e) {
		...
		if(manager.getProjektPersistence().Load(projektWin.getNeueName().getText()).getId()==0) {
			...
			JOptionPane.showMessageDialog(bearbeitenFenster, 
					"Die Projekt ist bereits vorhanden, überprüfen Sie den Namen.", 
					"Projekten", 
...
//			if(e.getID() == KeyEvent.KEY_RELEASED)
	...
	/*
	 * Tastatuer Events für Hauptfenster.
	 */
...
//				break;
	...
	/*
	 * Tastatuer Events für Login Fenster.
	 */	
	...
	/*
	 * Tastatuer Events für Bearbeiten Fenster.
	 */	
...
//					bearbeitenFenster.setVisible(false);
	...
	/*
	 * Tastatuer Events für Speichern Fenster.
	 */	
